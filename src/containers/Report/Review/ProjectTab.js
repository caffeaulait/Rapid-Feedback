...
class ProjectTab extends React.Component {
  render() {
    let scoreGet = this.props.result
      .map((a) => a.point)
      .reduce((a, b) => a + b, 0);
    let scoreTotal = this.props.result
      .map((a) => a.weight)
      .reduce((a, b) => a + b, 0);
    // eslint-disable-next-line no-unused-vars
    let dummy = this.props.markers.filter(
      (marker) => Number(marker.id) === Number(this.props.id)
    );
    ...
    options.push(
      <option value='Choose here' key={9000000} disabled hidden>
        Choose here
      </option>
    );
    options.push(
    ...
    let ids = this.props.studentInfo.students.map((student) => {
      return (
        <div className={styles.content} key={student.id}>
    ...
    return (
      <div>
        <div className={styles.label}>
          {this.props.studentInfo.group === 0 ? 'Student Name:' : 'Group Name:'}{' '}
        </div>
        <div className={styles.content}>
          {this.props.studentInfo.targetName}
        </div>
        <div className={styles.label}>
          {this.props.studentInfo.group === 0 ? 'Student ID:' : 'Student IDs:'}{' '}
        </div>
        ...
        <div className={styles.content}>
          {this.props.studentInfo.projectName}
        </div>
        ...
        <div className={styles.content}>
          <select
            ...
            onChange={(e) => this.props.setMarker(e.target.value)}
          >
            {options}
        ...
        <div className={styles.content}>
          {scoreGet.toFixed(2) + ' / ' + scoreTotal.toFixed(2)}
        </div>
      </div>
    );
  }

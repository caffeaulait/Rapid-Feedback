...
class Review extends React.Component {
    ...
    result: [
      {
        ...
        point: 0,
        comment: '1',
        criteria: 'Voice, peace and confidence',
        weight: 10,
      },
      {
        id: '2',
        point: 5,
        comment: '2',
        criteria: 'Knowledge of Material',
        weight: 10,
      },
  ...
  componentDidMount() {
    ...
    if (pathArray[pathArray.length - 3] === 'groups') {
      ...
      let gid = 0;
      let students = this.props.students.filter(
        (student) => student.id == pathArray[pathArray.length - 2]
      );
      array = students.map((s) => {
        return { name: s.first_name + ' ' + s.last_name, id: s.id };
      });
    ...
    let markers = this.props.backupresult.map((r) => ({
      id: r.markerId,
      name: r.results[0].firstName + ' ' + r.results[0].lastName,
    }));
    ...
    this.setState({
      ...
      assessDate: backupResult[0].assessDate,
      result: backupResult[0].result,
    });
  }
  ...
  continue = () => {
    ...
    let scoreGet = this.state.result
      .map((a) => a.point)
      .reduce((a, b) => a + b, 0);
  ...
  setCurrentMarker = (e) => {
    if (Number(e) === Number(999)) {
      ...
      let marker = this.state.markers.filter(
        (marker) => Number(marker.id) === Number(e)
      );
      let id = marker[0].id;
  ...
  handleUpdatePoint = (value, id, markerId) => {
    ...
    array[index] = {
      ...
      criteria: target[0].criteria,
      weight: target[0].weight,
    };
...
const mapDispatchToProps = (dispatch) => {
  return {
    ...
    upDateResults: (mid, pid, sid, assessList, assessedDate, gid) => {
      dispatch(
        actions.onUpdateResult(mid, pid, sid, assessList, assessedDate, gid)
      );
    },
  };
};

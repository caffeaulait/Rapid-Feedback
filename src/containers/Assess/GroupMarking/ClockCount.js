    ...
    render() {
        ...
        return (
            <span className = "clock">
                <img onClick = {this.props.start} src={Clock} />
                { this.props.minutes === 0 && this.props.seconds === 0
                    ? <label className = "timeOut time" >Time out</label>
                    : <label className = "timeRemain time">Time Remaining: {this.props.minutes}:{this.props.seconds < 10 ? `0${this.props.seconds}` : this.props.seconds}</label>
                }
            </span>
        )
    }
